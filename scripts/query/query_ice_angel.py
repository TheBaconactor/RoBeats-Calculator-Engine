...
# Find Ice Angel (Easy)
cursor = conn.execute(
    "SELECT name, best_score, best_fg_score FROM songs WHERE LOWER(name) LIKE ? ORDER BY name", ("%ice angel%easy%",)
)
...
if not results:
    ...
    cursor = conn.execute(
        "SELECT name, best_score, best_fg_score FROM songs WHERE LOWER(name) LIKE ? ORDER BY name", ("%ice angel%",)
    )
    results = cursor.fetchall()

if not results:
    print("No songs found matching 'ice angel'")
    conn.close()
    raise SystemExit(0)
