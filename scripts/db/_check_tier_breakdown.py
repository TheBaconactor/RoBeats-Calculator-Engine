...
tiers = c.execute(
    'SELECT team_buff, score, fg_score FROM team_buff_loadouts WHERE song_name=? ORDER BY team_buff',
    (song_name,)
).fetchall()
...
fg_tiers = c.execute(
    'SELECT team_buff, fg_score FROM team_buff_fg_loadouts WHERE song_name=? ORDER BY team_buff',
    (song_name,)
).fetchall()
if fg_tiers:
