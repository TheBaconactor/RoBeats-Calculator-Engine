#!/usr/bin/env python3
"""Measure COLD frontier-cache build time (base timeline + FG response) on a
stratified sample and extrapolate to the full pool.

Builds into fresh temp cache dirs (TIMELINE_/FG_RESPONSE_FRONTIER_CACHE_DIR set
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Measure cold frontier-cache build time and extrapolate to the pool.")
    ...
    def _build(queue: list[tuple], label: str) -> tuple[float, float, object, object]:
        ...
        fg_s = time.perf_counter() - t1
        print(f"  [{label}] timeline built={tl.built} disk={tl.disk} fail={tl.failures} {tl_s:.1f}s   "
              f"FG built={fg.built} disk={fg.disk} fail={fg.failures} {fg_s:.1f}s")
        return tl_s, fg_s, tl, fg
    ...
    for d in DIFFS:
        ...
        tl_s, fg_s, tl, fg = _build(queue, d)
        n = max(1, int(tl.built) or len(queue))
        rows.append((d, n, tl_s, fg_s, pool_counts[d]))
        tl_total += tl_s
        fg_total += fg_s
        n_total += n
    ...
    tl_pool = fg_pool = 0.0
    for d, n, tl_s, fg_s, pc in rows:
        tl_rate, fg_rate = tl_s / n, fg_s / n
        tl_pool += tl_rate * pc
        fg_pool += fg_rate * pc
        print(f"  {d:6s} n={n:3d}  timeline {tl_rate*1000:5.0f} ms/song  FG {fg_rate*1000:6.0f} ms/song"
              f"   -> pool {pc}: timeline {tl_rate*pc:5.0f}s  FG {fg_rate*pc:6.0f}s")
    ...
    print(f"  TOTAL         : ~{(tl_pool+fg_pool):6.0f}s  (~{(tl_pool+fg_pool)/60:.1f} min)")
