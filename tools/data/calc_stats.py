...
for name in gear_names:
    g = gears.get(name, {})
    print(
        f"{name}: PP={g.get('Perfect Points', 0)} CM={g.get('Combo Multiplier', 0)} FM={g.get('Fever Multiplier', 0)} FT={g.get('Fever Time', 0)} FF={g.get('Fever Fill Rate', 0)} Vibe={g.get('Vibe', 0)}"
    )
    for k in stats:
        stats[k] += g.get(k, 0)
...
for name in mini_names:
    m = minis.get(name, {})
    print(
        f"{name}: PP={m.get('Perfect Points', 0)} CM={m.get('Combo Multiplier', 0)} FM={m.get('Fever Multiplier', 0)} FT={m.get('Fever Time', 0)} FF={m.get('Fever Fill Rate', 0)} Vibe={m.get('Vibe', 0)}"
    )
    for k in stats:
        stats[k] += m.get(k, 0)
