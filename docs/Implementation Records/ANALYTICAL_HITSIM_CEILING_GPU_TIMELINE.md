...
- New API helper that builds representative maps with float32-matching math:
  - `gear_optimizer/solver/taichi_gem/api/timeline.py::_build_ceiling_cell_rep_maps`
    - Returns 1D rep maps (`rep_ft_by_ft`, `rep_ff_by_ff`) and rep index lists (`rep_ft_values`, `rep_ff_values`).
- New Taichi kernels:
  - `compute_timeline_grid_ceiling_hitsim_reps_kernel(...)` (compute reps only; takes rep index lists)
  - `scatter_timeline_grid_ceiling_hitsim_from_reps_kernel(...)` (fill full grid; takes 1D rep maps)
- Wiring:
