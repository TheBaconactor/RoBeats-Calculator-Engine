...
from reverse_score_v2.domain import Loadout, Tables, compose_stats
from reverse_score_v2.game_model import gear_power
...
class SongOracle:
    ...
    def song_colors(self) -> tuple[str, ...]:
        # Keep secondary even when it equals primary. Charts like Gateway /
        # Sky Blue publish Secondary Color == Primary Color; ExactScoreIR
        # reads both slots from the same stats key (base_int = 3*c1). Collapsing
        # to a 1-tuple made DomainIR project secondary=0 and under-score by ~1/3
        # of color mass relative to score_stats_exact.
