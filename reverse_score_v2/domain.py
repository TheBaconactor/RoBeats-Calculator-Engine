"""Loadout domain for the reverse engine v2: gear, upgrades, minis, gems, buffs.

Adapted from v1 ``reverse_score.domain`` to reuse production extractors:

- ``gear_optimizer.data.mini_scaling`` -- PetDef + PetUtils scaling
- ``gear_optimizer.data.upgrades`` -- UpgradeDef + per-piece cap
- ``gear_optimizer.data.mini_ascension`` -- song-target materialization
"""
...
from gear_optimizer.data.upgrades import UPGRADES_PER_PIECE_MAX, UpgradeDef

from reverse_score_v2.game_model import COLOR_KEYS, merge_stats
...
class Loadout:
    ...
    team_buff: tuple[str, str] | None = None
...
def build_tables(
    *,
    ...
    return Tables(
        ...
        upgrades_by_id={u.uid: u for u in upgrades},
    )
...
def gem_stats_delta(gems: GemAlloc) -> dict[str, int]:
    """Optimizer-canonical gem scaling."""
    delta: dict[str, int] = {}
...
def _upgrade_stats(upgrade: UpgradeDef) -> dict[str, int]:
    return dict(upgrade.stat_pattern)
...
def compose_stats(
    ...
    *,
    ...
    """Loadout -> total statsdict (gear + upgrades + buff + minis + gems)."""
    ...
    for slot in GEAR_SLOTS:
        ...
        for uid in loadout.upgrades.get(slot, ()):
            merge_stats(stats, _upgrade_stats(tables.upgrades_by_id[uid]))
