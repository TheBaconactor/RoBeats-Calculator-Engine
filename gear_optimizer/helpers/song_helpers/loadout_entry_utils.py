...
def canonicalize_loadout_ids(loadout_ids: Any) -> tuple[int, ...] | None:
    if loadout_ids is None:
        return None
    try:
        ids = [int(x) for x in list(loadout_ids)[:9]]
    except Exception as e:
        logger.debug(f"loadout_entry_utils:canonicalize_loadout_ids: {e}")
        return None
    if len(ids) < 9:
        return None
    gear_ids = ids[:6]
...
def candidate_id_key(loadout_ids: Any) -> str | None:
    canon = canonicalize_loadout_ids(loadout_ids)
    if canon is None:
        return None
    return "ids:" + ",".join(str(x) for x in canon)


def candidate_loadout_ids(candidate: Any) -> tuple[int, ...] | None:
    if not isinstance(candidate, dict):
        return None
    return canonicalize_loadout_ids(candidate.get("LoadoutIDs"))


def entry_loadout_ids(entry: Any) -> tuple[int, ...] | None:
    if not isinstance(entry, dict):
        return None
    ids = entry.get("loadout_ids")
    if ids is None:
        ids = entry.get("LoadoutIDs")
    if ids is None:
        eval_data = entry.get("eval_data")
        if isinstance(eval_data, dict):
            ids = eval_data.get("LoadoutIDs")
    return canonicalize_loadout_ids(ids)


def _decode_names_from_registry(registry: Any, loadout_ids: tuple[int, ...]) -> list[str]:
    ids_arr = np.asarray(loadout_ids, dtype=np.int32)
    ...
    if callable(decode_names):
        try:
            ...
            logger.debug(f"loadout_entry_utils:_decode_names_from_registry: {e}")
    decode_loadout = getattr(registry, "decode_loadout", None)
    if callable(decode_loadout):
        try:
            return names_list(decode_loadout(ids_arr))
        except Exception as e:
            logger.debug(f"loadout_entry_utils:_decode_names_from_registry: {e}")
    return []
...
def materialize_candidate_names(
    ...
    *,
    ...
    names = _decode_names_from_registry(registry_obj, loadout_ids)
    if len(names) < 9:
        return [], []
...
def candidate_loadout_hash(
    ...
    *,
    ...
    if not gear_names and not mini_names:
        return candidate_id_key(candidate.get("LoadoutIDs"))
    ...
    if (primary_color or secondary_color) and isinstance(minis_by_name, dict):
        try:
            ...
            logger.debug(f"loadout_entry_utils:_remember: {e}")
...
def materialize_entry_names(entry: Any, *, mutate: bool = True) -> tuple[list[str], list[str]]:
    ...
    loadout_ids = entry_loadout_ids(entry)
    if loadout_ids is None:
        return [], []

    registry_obj = entry.get("_item_registry")
    if registry_obj is None:
        return [], []
...
def entry_loadout_hash(entry: Any) -> str | None:
    ...
    try:
        ...
        logger.debug(f"loadout_entry_utils:entry_loadout_hash: {e}")
        return None
