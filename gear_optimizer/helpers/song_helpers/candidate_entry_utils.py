...
def canonicalize_genome_ids(genome_ids: Any) -> tuple[int, ...] | None:
    ...
    try:
        ...
        logger.debug(f"candidate_entry_utils:canonicalize_genome_ids: {e}")
        return None
    if len(ids) < 9:
        return None
    gear_ids = ids[:6]
...
def candidate_key(genome_ids: Any) -> str | None:
...
def _decode_names_from_registry(registry: Any, genome_ids: tuple[int, ...]) -> list[str]:
    ...
    if callable(decode_names):
        try:
            ...
            logger.debug(f"candidate_entry_utils:_decode_names_from_registry: {e}")
...
def candidate_loadout_hash(
    ...
    *,
    ...
    if not gear_names and not mini_names:
        return candidate_key(candidate.get("GenomeIDs"))
    ...
    if (primary_color or secondary_color) and isinstance(minis_by_name, dict):
        try:
            ...
            logger.debug(f"candidate_entry_utils:_remember: {e}")
...
def entry_loadout_hash(entry: Any) -> str | None:
    ...
    try:
        ...
        logger.debug(f"candidate_entry_utils:entry_loadout_hash: {e}")
        return None
