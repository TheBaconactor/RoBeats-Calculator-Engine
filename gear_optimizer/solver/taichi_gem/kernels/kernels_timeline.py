...
- compute_timeline_grid_kernel: Parallel 161x161 timeline grid computation
- compute_timeline_grid_ceiling_envelope_kernel: Analytical timing-envelope timeline grid computation
- compute_timeline_grid_ceiling_envelope_reps_kernel: Representative-cell ceiling precompute
- scatter_timeline_grid_ceiling_envelope_from_reps_kernel: Scatter representative outputs across the grid
- compute_timeline_grid_signatures_kernel: Recompute compact signatures from existing grid outputs
- unpack_timeline_grid_masks_kernel: Optional debug unpack of bitpacked masks

The timeline grid kernels precompute all valid fever timelines for the full FT/FF stat
range (0-160 for each), enabling O(1) lookups during gem optimization instead of
recomputing timelines for each combination.

This is a critical performance optimization that reduces timeline computation from
O(n_combos * song_notes) to O(1) per combo after precomputation.
"""
