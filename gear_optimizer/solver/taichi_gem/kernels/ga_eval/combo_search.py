...
def ga_find_best_combo_key_kernel(
    ...
    if ti.static(IS_METAL):
        ...
        # Taichi does not guarantee that `tid % block_dim` maps to the physical `thread_idx` on Vulkan,
        # which can silently produce incorrect winners when `n_genomes>1`.
        #
        # Atomic reduction is correct; the result materialization kernel will recompute the final gem
        # allocation when the cached `chunk_best_results` payload is missing/invalid.
        ti.loop_config(block_dim=kernels_helpers._KERNEL_BLOCK_DIM)
        for genome_idx, local_c in ti.ndrange(n_genomes, combo_count):
            combo_idx: ti.i32 = combo_offset + local_c
            if combo_idx >= n_combos:
