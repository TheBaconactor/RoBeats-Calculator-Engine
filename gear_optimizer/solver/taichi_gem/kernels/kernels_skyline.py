...
This module contains 8 kernels implementing Skyline candidate operators:
- skyline_seed_rng_kernel: Initialize per-genome RNG state
- skyline_select_parents_tournament_kernel: Tournament selection
- SKYLINE_crossover_mutate_kernel: Crossover + mutation + mini uniqueness
- SKYLINE_swap_populations_kernel: Copy next generation to current
- skyline_copy_elites_kernel: Preserve elite solutions
- skyline_aggregate_genome_stats_kernel: Aggregate item stats into genome stats
- skyline_copy_scores_kernel: Bridge evaluation to selection

These kernels enable fully GPU-native skyline execution, avoiding CPU-GPU transfers
...
_SKYLINE_DIVERSE_PARENT_B_RATE = max(0.0, min(1.0, env_float("GPU_SKYLINE_DIVERSE_PARENT_B_RATE", 0.125)))
_SKYLINE_DIVERSE_PARENT_B_RATE_FP = int(_SKYLINE_DIVERSE_PARENT_B_RATE * 4294967295.0)
...
def _exact_eval_key_matches(pos: ti.i32, genome_idx: ti.i32, n_slots: ti.i32) -> ti.i32:
    ...
    for s in ti.static(range(9)):
        ...
        if kernels_helpers.skyline_exact_eval_hash_keys[pos, s] != want:
            match = 0
    return match
...
def _store_exact_eval_key(pos: ti.i32, genome_idx: ti.i32, n_slots: ti.i32) -> None:
    for s in ti.static(range(9)):
        ...
        kernels_helpers.skyline_exact_eval_hash_keys[pos, s] = value
...
def _exact_eval_base_stats_key_matches(pos: ti.i32, genome_idx: ti.i32) -> ti.i32:
    ...
    for i in ti.static(range(7)):
        ...
        if kernels_helpers.skyline_exact_eval_hash_keys[pos, i] != want:
            match = 0
    return match
...
def _store_exact_eval_base_stats_key(pos: ti.i32, genome_idx: ti.i32) -> None:
    for i in ti.static(range(7)):
        kernels_helpers.skyline_exact_eval_hash_keys[pos, i] = ti.cast(
            kernels_helpers.genome_base_stats[genome_idx][i], ti.i32
        )
    for i in ti.static(range(7, 9)):
        kernels_helpers.skyline_exact_eval_hash_keys[pos, i] = 0


@ti.kernel
def _SKYLINE_prepare_exact_eval_reuse_sort_arrays_kernel(n_genomes: ti.i32, n_slots: ti.i32):
    """
    Prepare hash keys + indices for parallel sort grouping.

    The sorted arrays are consumed by `_SKYLINE_build_exact_eval_reuse_map_from_sorted_kernel`.
    """
    sentinel = ti.i32(2147483647)
    for i in range(kernels_helpers.skyline_exact_eval_hash_sort_keys.shape[0]):
        if i < n_genomes:
            h = _hash_exact_eval_input_for_genome(i, n_slots)
            kernels_helpers.skyline_exact_eval_hash_sort_keys[i] = ti.cast(h, ti.i32)
            kernels_helpers.skyline_exact_eval_hash_sort_indices[i] = i
        else:
            kernels_helpers.skyline_exact_eval_hash_sort_keys[i] = sentinel
            kernels_helpers.skyline_exact_eval_hash_sort_indices[i] = -1
    kernels_helpers.skyline_exact_eval_unique_count[0] = 0


@ti.kernel
def _SKYLINE_build_exact_eval_reuse_map_from_sorted_kernel(n_genomes: ti.i32, n_slots: ti.i32):
    """
    Build `skyline_exact_eval_rep_idx` after sorting hash keys.
    ...
    for g in range(n_genomes0):
        kernels_helpers.skyline_exact_eval_rep_idx[g] = g

    n_total = kernels_helpers.skyline_exact_eval_hash_sort_keys.shape[0]
    ti.loop_config(block_dim=kernels_helpers._KERNEL_BLOCK_DIM)
    for i in range(n_total):
        g = kernels_helpers.skyline_exact_eval_hash_sort_indices[i]
        if g < 0 or g >= n_genomes0:
            continue

        key = kernels_helpers.skyline_exact_eval_hash_sort_keys[i]
        rep = g

        j = i - 1
        while j >= 0 and kernels_helpers.skyline_exact_eval_hash_sort_keys[j] == key:
            cand = kernels_helpers.skyline_exact_eval_hash_sort_indices[j]
        ...
        j = i + 1
        while j < n_total and kernels_helpers.skyline_exact_eval_hash_sort_keys[j] == key:
            cand = kernels_helpers.skyline_exact_eval_hash_sort_indices[j]
            if cand >= 0 and cand < n_genomes0:
                if _exact_eval_input_matches_genomes(cand, g, n_slots) != 0:
                    rep = ti.min(rep, cand)
        ...
        kernels_helpers.skyline_exact_eval_rep_idx[g] = rep
        if rep == g:
            ti.atomic_add(kernels_helpers.skyline_exact_eval_unique_count[0], 1)


@ti.kernel
def _SKYLINE_build_exact_eval_reuse_map_serial_kernel(n_genomes: ti.i32, n_slots: ti.i32):
    """
    Original open-addressing implementation (serialized).

    Kept for the current Metal path and as a correctness fallback.
    """
    for i in range(kernels_helpers.skyline_exact_eval_hash_used.shape[0]):
        kernels_helpers.skyline_exact_eval_hash_used[i] = 0
    kernels_helpers.skyline_exact_eval_unique_count[0] = 0

    ti.loop_config(serialize=True)
    for g in range(n_genomes):
        h = _hash_exact_eval_input_for_genome(g, n_slots)
        mask = kernels_helpers.skyline_exact_eval_hash_used.shape[0] - 1
        pos = ti.cast(h & ti.u32(mask), ti.i32)
        rep = g
        handled = ti.i32(0)

        for _ in range(kernels_helpers.skyline_exact_eval_hash_used.shape[0]):
            entry = kernels_helpers.skyline_exact_eval_hash_used[pos]
            if entry == 0:
                kernels_helpers.skyline_exact_eval_hash_used[pos] = g + 1
                _store_exact_eval_key(pos, g, n_slots)
                kernels_helpers.skyline_exact_eval_unique_count[0] = kernels_helpers.skyline_exact_eval_unique_count[0] + 1
                rep = g
                handled = 1
                break
...
def SKYLINE_build_exact_eval_reuse_map_kernel(n_genomes: int, n_slots: int) -> None:
    """
    ...
    if n_genomes_i <= 1:
        kernels_helpers.skyline_exact_eval_unique_count[0] = 0
        if n_genomes_i == 1:
            kernels_helpers.skyline_exact_eval_rep_idx[0] = 0
    ...
    _SKYLINE_prepare_exact_eval_reuse_sort_arrays_kernel(n_genomes_i, n_slots_i)
    from taichi.algorithms import parallel_sort as _parallel_sort

    _parallel_sort(kernels_helpers.skyline_exact_eval_hash_sort_keys, kernels_helpers.skyline_exact_eval_hash_sort_indices)
    _SKYLINE_build_exact_eval_reuse_map_from_sorted_kernel(n_genomes_i, n_slots_i)


@ti.kernel
def _SKYLINE_prepare_exact_eval_base_stats_reuse_sort_arrays_kernel(n_genomes: ti.i32):
    sentinel = ti.i32(2147483647)
    for i in range(kernels_helpers.skyline_exact_eval_hash_sort_keys.shape[0]):
        if i < n_genomes:
...
@ti.kernel
def _SKYLINE_build_exact_eval_reuse_map_from_base_stats_sorted_kernel(n_genomes: ti.i32):
    n_genomes0 = ti.max(ti.i32(0), n_genomes)
    for g in range(n_genomes0):
        kernels_helpers.skyline_exact_eval_rep_idx[g] = g
...
@ti.kernel
def _SKYLINE_build_exact_eval_reuse_map_from_base_stats_serial_kernel(n_genomes: ti.i32):
    for i in range(kernels_helpers.skyline_exact_eval_hash_used.shape[0]):
        kernels_helpers.skyline_exact_eval_hash_used[i] = 0
    kernels_helpers.skyline_exact_eval_unique_count[0] = 0
...
def SKYLINE_build_exact_eval_reuse_map_from_base_stats_kernel(n_genomes: int) -> None:
    """
    ...
    _SKYLINE_prepare_exact_eval_base_stats_reuse_sort_arrays_kernel(n_genomes_i)
    from taichi.algorithms import parallel_sort as _parallel_sort

    _parallel_sort(kernels_helpers.skyline_exact_eval_hash_sort_keys, kernels_helpers.skyline_exact_eval_hash_sort_indices)
    _SKYLINE_build_exact_eval_reuse_map_from_base_stats_sorted_kernel(n_genomes_i)


@ti.kernel
def SKYLINE_propagate_exact_eval_reuse_base_stats_kernel(n_genomes: ti.i32):
...
@ti.kernel
def SKYLINE_propagate_exact_eval_reuse_chunk_best_kernel(n_genomes: ti.i32):
...
@ti.kernel
def skyline_seed_rng_kernel(n_genomes: ti.i32, seed: ti.u32):
    """
    ...
    for g in range(n_genomes):
        ...
        if s == ti.u32(0):
            s = ti.u32(1)
        kernels_helpers.skyline_rng_state[g] = s


@ti.kernel
def skyline_seed_rng_runs_kernel(n_genomes_total: ti.i32, n_genomes_per_run: ti.i32, seed: ti.u32):
    """
    Initialize per-genome RNG state for multiple independent runs packed contiguously.

    This mirrors skyline_seed_rng_kernel, but seeds each run as if its genomes were indexed
...
@ti.kernel
def skyline_load_initial_populations_batch_kernel(
    ...
    The output layout is contiguous by run:
      ...
      population_indices[g] = skyline_initial_populations[run_idx_start + run, local]
    """
    ti.loop_config(block_dim=kernels_helpers._KERNEL_BLOCK_DIM)
    if n_genomes_per_run > 0 and n_runs > 0:
        ...
        for g in range(n_total):
            ...
            for s in range(n_slots):
                kernels_helpers.population_indices[g, s] = kernels_helpers.skyline_initial_populations[src_run, local_g, s]
...
@ti.kernel
def skyline_select_parents_tournament_kernel(n_genomes: ti.i32, tournament_k: ti.i32):
    """
    ...
    Produces two parent indices per output genome (skyline_parent_a/b).
    ...
    ti.loop_config(block_dim=kernels_helpers._KERNEL_BLOCK_DIM)
    for g in range(n_genomes):
        state = kernels_helpers.skyline_rng_state[g]
        ...
        for _ in range(tournament_k):
            ...
            sc = kernels_helpers.skyline_scores[idx]
        ...
        if ti.static(_SKYLINE_DIVERSE_PARENT_B_RATE_FP > 0):
            if state < ti.u32(_SKYLINE_DIVERSE_PARENT_B_RATE_FP):
        ...
        kernels_helpers.skyline_parent_a[g] = pa
        kernels_helpers.skyline_parent_b[g] = pb
        kernels_helpers.skyline_rng_state[g] = state


@ti.kernel
def SKYLINE_crossover_mutate_kernel(
    n_genomes: ti.i32,
    n_slots: ti.i32,
    ...
    Applies:
    1. Uniform crossover from two parents (provided via skyline_parent_a/skyline_parent_b)
    ...
    Note: Elites are handled separately via skyline_copy_elites_kernel (call after this).
    ...
    for g in range(n_genomes):
        state = kernels_helpers.skyline_rng_state[g]
        pa = kernels_helpers.skyline_parent_a[g]
        pb = kernels_helpers.skyline_parent_b[g]
...
@ti.kernel
def SKYLINE_swap_populations_kernel(n_genomes: ti.i32, n_slots: ti.i32):
    """
    Copy population_next_indices -> population_indices in-place.

    This advances the skyline to the next generation.
...
@ti.kernel
def skyline_copy_elites_kernel(
    n_elites: ti.i32,
    n_slots: ti.i32,
...
@ti.kernel
def skyline_copy_island_elites_kernel(
    n_elites: ti.i32,
    n_slots: ti.i32,
):
    """
    ...
    skyline_find_island_elites_kernel) instead of a CPU ndarray. This avoids the
...
@ti.kernel
def skyline_copy_scores_kernel(n_genomes: ti.i32):
    """
    Copy scores from genome_result_stats to skyline_scores for selection.

    This bridges the evaluation kernel output to the skyline selection input.
    skyline_scores[g] = genome_result_stats[g][0] (the score component)
    ...
    ti.loop_config(block_dim=kernels_helpers._KERNEL_BLOCK_DIM)
    for g in range(n_genomes):
        kernels_helpers.skyline_scores[g] = kernels_helpers.genome_result_stats[g][0]


@ti.kernel
def skyline_select_crossover_mutate_kernel(
    n_genomes: ti.i32,
    n_slots: ti.i32,
    tournament_k: ti.i32,
    ...
    Combines skyline_select_parents_tournament_kernel + SKYLINE_crossover_mutate_kernel
    to reduce kernel launch overhead.
...
@ti.kernel
def skyline_next_generation_full_kernel(
    n_genomes: ti.i32,
    n_slots: ti.i32,
    n_elites: ti.i32,
    tournament_k: ti.i32,
...
@ti.kernel
def skyline_next_generation_full_islands_kernel(
    n_genomes: ti.i32,
    n_slots: ti.i32,
    n_islands: ti.i32,
    ...
    """
    FUSED next generation with on-the-fly island elitism (no skyline_find_island_elites kernel required).

    This kernel replaces the sequence:
      skyline_find_island_elites() -> skyline_next_generation_full_kernel()
    ...
    Notes:
    - We intentionally mirror the tie-breaking behavior of skyline_find_island_elites_kernel:
            ...
            if isl >= n_islands_i:
                ...
                if elite_rank >= 0:
                    ...
                    for local_idx in range(isl_size):
                        ...
                        score: ti.i32 = kernels_helpers.skyline_scores[idx]
...
@ti.func
def _skyline_next_generation_full_runs_impl(
    n_runs: ti.i32,
    n_genomes_per_run: ti.i32,
    n_slots: ti.i32,
    n_islands: ti.i32,
...
@ti.kernel
def skyline_next_generation_full_runs_kernel(
    n_runs: ti.i32,
    n_genomes_per_run: ti.i32,
    n_slots: ti.i32,
    n_islands: ti.i32,
    ...
    """
    ti.loop_config(block_dim=kernels_helpers._KERNEL_BLOCK_DIM)
    _skyline_next_generation_full_runs_impl(
        n_runs,
        n_genomes_per_run,
        n_slots,
        n_islands,
...
@ti.kernel
def skyline_refresh_scores_update_runs_best_and_next_generation_full_runs_kernel(
    run_idx_start: ti.i32,
    n_runs: ti.i32,
    n_genomes_per_run: ti.i32,
    n_slots: ti.i32,
    ...
    - refresh `skyline_scores` from the exact reduction key
    - preserve per-run row 0 before mutation when a run improves
    - build the next generation in the same dispatch

    This removes the tiny refresh launch from non-final, non-migration skyline generations while
...
@ti.kernel
def SKYLINE_swap_population_kernel(n_genomes: ti.i32, n_slots: ti.i32):
    """
