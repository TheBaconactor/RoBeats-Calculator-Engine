...
def _fg_response_inner_pattern_batch_kernel(
    ...
    for pair_row in range(pair_count_total):
        ...
        # The score upper bound depends on a body's retained state only through body_fever;
        # for fixed allocation/head inputs it is affine in that coordinate. Its exact maximum
        # over this pair is therefore attained at the pair's minimum or maximum body_fever.
        # Derive those extrema once, then keep every allocation-time bound O(1).
        body_fever_min: ti.i32 = 0x7FFFFFFF
        body_fever_max: ti.i32 = -1
        pair_pos: ti.i32 = 0
        while pair_pos < pair_count:
            ...
            body_fever_min = ti.min(body_fever_min, body_fever)
            body_fever_max = ti.max(body_fever_max, body_fever)
            pair_pos += 1
        assert body_fever_min <= body_fever_max
        ...
        while g_cm <= max_cm_gems:
            ...
            while g_fm <= g_fm_max:
                ...
                pair_ub: FP = _fg_response_surface_upper_bound(
                    max_base_value,
                    cm_mul,
                    fm_mul,
                    body_fever_min,
                    ti.max(0, body_total - body_fever_min),
                    n_hn,
                    n_hf,
                    sigma_hn,
                    sigma_hf,
                ...
                pair_ub = ti.max(
                    ...
                    _fg_response_surface_upper_bound(
                        ...
                        fm_mul,
                        body_fever_max,
                        ti.max(0, body_total - body_fever_max),
                        n_hn,
                        n_hf,
                        sigma_hn,
                        sigma_hf,
                    ...
                    if ti.static(allow_pp_template):
                        if max_pp_gems > 0:
                            ...
                            while g_pp <= g_pp_max:
                                ...
                                pp_pair_ub: FP = _fg_response_surface_upper_bound(
                                    pp_base_value,
                                    cm_mul,
                                    fm_mul,
                                    body_fever_min,
