...
print('Entries with #include signal:', c.execute("SELECT COUNT(*) FROM loadouts WHERE song_name LIKE '%signal%'").fetchone()[0])

# Check for the specific ones still broken
bad_rowids = [16207, 16213, 16214, 16216, 16221, 16222, 16228, 16229, 16230, 122389, 155653, 155670, 155671, 155672, 155678, 155679, 155710, 155712]
