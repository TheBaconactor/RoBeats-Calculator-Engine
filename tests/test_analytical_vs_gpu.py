    ...
    try:
        ...
        for ft_stat, ff_stat in test_points:
            ...
            if not match:
                ...
                mismatches.append({
                    ...
                    'gt_bf': gt_bf, 'gt_bn': gt_bn,
                    'an_bf': an_bf, 'an_bn': an_bn,
                })
            
            print(f"   FT={ft_stat:3d}, FF={ff_stat:3d}: acts={gt_acts} | "
        ...
        if acts >= 1:
            for forced_counts in [[0], [5], [10], [20]]:
                ...
                print(f"   Config {str(forced_counts_padded):15s}: "
                      f"GT(bf={gt_bf:4d}, bn={gt_bn:4d}) vs "
                      f"AN(bf={an_bf:4d}, bn={an_bn:4d}) {status}")
