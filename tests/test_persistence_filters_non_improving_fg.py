...
def test_build_db_payload_drops_fg_variants_with_empty_normalized_force_payload(monkeypatch):
    ...
    monkeypatch.setattr(
        "gear_optimizer.helpers.song_helpers.persistence_payload.normalize_force_payload",
        lambda force_obj: {},
    )
    ...
    fg_variants = [
        {
            ...
            "base_score": 50,
            "fg_score": 150,
            "data": {"Score": 150, "ForceGreats": {"config": {"NonFever1": 2}}},
        }
    ]
    ...
    assert payload.get("fg_score", 0) == 0
    assert payload.get("force") is None
    assert payload.get("best_fg") is None
