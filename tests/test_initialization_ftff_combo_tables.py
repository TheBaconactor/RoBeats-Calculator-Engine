...
def test_ensure_ftff_combo_tables_order_and_cache(monkeypatch):
    ...
    init._FTFF_COMBO_CACHE = {"key": None, "n_combos": 0}
...
def test_ensure_ftff_combo_tables_respects_global_caps(monkeypatch):
    ...
    init._FTFF_COMBO_CACHE = {"key": None, "n_combos": 0}
    n = init._ensure_ftff_combo_tables(3, max_ft_gems=1, max_ff_gems=2)
    assert n == 6

    ft = uploads["ft"][:n]
    ff = uploads["ff"][:n]
    expected_ft = np.array([0, 0, 0, 1, 1, 1], dtype=np.int32)
    expected_ff = np.array([0, 1, 2, 0, 1, 2], dtype=np.int32)
    assert np.array_equal(ft, expected_ft)
    assert np.array_equal(ff, expected_ff)

    calls_after_first = int(uploads["calls"])
    n_again = init._ensure_ftff_combo_tables(3, max_ft_gems=1, max_ff_gems=2)
    assert n_again == 6
    assert int(uploads["calls"]) == calls_after_first
