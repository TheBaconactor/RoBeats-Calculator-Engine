...
def test_zero_ms_both_batch_does_not_attach_force_to_base_only_rows(monkeypatch):
    from gear_optimizer.helpers.song_helpers import team_buff_tiers
    ...
    monkeypatch.setattr(
        "gear_optimizer.solver.solver_common.build_candidate_payload_exact_cpu",
        lambda **_kwargs: {
            "Score": 100,
            "Stats": {"Perfect Points": 25, "Beat": 100},
            "GemCounts": {"Perfect Points": 0},
    ...
    monkeypatch.setattr(
        "gear_optimizer.helpers.song_helpers.fg_exact_resolve.entry_genome",
        lambda _entry: [],
    )
    monkeypatch.setattr(
        "gear_optimizer.solver.scoring.exact_rescore.score_stats_fixed_timing_exact_batch",
        lambda rows, *_args, **_kwargs: [100 for _row in rows],
    )
    monkeypatch.setattr(
        "gear_optimizer.data.csv_parser.get_fixed_stats",
        lambda _cfg: {
    ...
    batches = team_buff_tiers.build_team_buff_tier_db_batches(
        entries=[_entry("noop"), _entry("real-fg")],
        calc_song={
            "metadata": {
                "Song Name": "pytest_zero_ms_fg_base_pairing",
    ...
    by_hash = {row["loadout_hash"]: row for row in batches["T5"]}
    assert by_hash["noop"]["score"] == 100
    assert by_hash["noop"]["fg_score"] == 0
    assert by_hash["noop"]["fg_base_score"] == 0
    assert by_hash["noop"]["force"] is None
    assert by_hash["real-fg"]["fg_score"] == 90
    assert by_hash["real-fg"]["fg_base_score"] == 80
    assert isinstance(by_hash["real-fg"]["force"], dict)
