"""Test loadout reconstruction optimization for correctness and performance.
...
class MockRegistry:
    ...
    def decode_loadout(self, loadout_ids):
        """Decode a loadout from item IDs."""
        return [self.items[idx] for idx in loadout_ids]
...
def mock_loadout_reconstruction_reference(
    scores, results, candidate_ids, registry, base_stats_fixed, cfg_data, top_indices, limit=50
):
    """
    Straightforward loadout reconstruction reference.
    """
    ...
    for idx in top_indices:
        ...
        loadout_ids = candidate_ids[idx]
        loadout = registry.decode_loadout(loadout_ids)
...
def mock_loadout_reconstruction_optimized(
    scores, results, candidate_ids, registry, base_stats_fixed, cfg_data, top_indices, limit=50
):
    """
    Optimized loadout reconstruction logic.
    ...
    for idx in top_indices:
        ...
        loadout = registry.decode_loadout(candidate_ids[idx])
        res_row = results[idx]
...
def test_data():
    ...
    # Create mock candidate results
    n_candidates = 100
    scores = np.random.randint(-10, 1000, n_candidates)

    # Create mock results array (candidate_id, g_ft, g_ff, g_pp, g_cm, g_fm, g_ov)
    results = np.zeros((n_candidates, 7), dtype=np.int32)
    results[:, 1:] = np.random.randint(0, 30, (n_candidates, 6))

    # Create mock loadout IDs
    candidate_ids = np.random.randint(0, 15, (n_candidates, 9), dtype=np.int32)
...
def test_reconstruction_correctness(test_data):
    ...
    # Run both versions
    original = mock_loadout_reconstruction_reference(**test_data)
    optimized = mock_loadout_reconstruction_optimized(**test_data)
...
def test_reconstruction_performance(test_data):
    ...
    for _ in range(10):
        mock_loadout_reconstruction_reference(**test_data)
        mock_loadout_reconstruction_optimized(**test_data)
    ...
    # Original timing
    orig_time = _time(mock_loadout_reconstruction_reference, loops=100)

    # Optimized timing
    opt_time = _time(mock_loadout_reconstruction_optimized, loops=100)
