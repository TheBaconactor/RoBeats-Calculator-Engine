...
def test_response_inner_pair_bound_is_maximized_at_body_fever_extrema():
    from gear_optimizer.solver.taichi_gem.force_greats.response_inner_host import (
        _fg_response_upper_bound_native_f64,
    )

    rng = np.random.default_rng(116)
    for _case in range(256):
        body_total = int(rng.integers(0, 8_000))
        body_values = np.unique(rng.integers(0, body_total + 1, size=16)).astype(np.int64)
        body_min = int(body_values[0])
        body_max = int(body_values[-1])
        args = (
            float(rng.uniform(1.0, 20_000.0)),
            float(rng.uniform(0.1, 8.0)),
            float(rng.uniform(0.1, 8.0)),
        )
        head_coeffs = tuple(int(v) for v in rng.integers(0, 20_000, size=4))

        def bound(body_fever: int) -> float:
            return float(
                _fg_response_upper_bound_native_f64(
                    *args,
                    int(body_fever),
                    int(body_total - body_fever),
                    *head_coeffs,
                )
            )

        endpoint_bound = max(bound(body_min), bound(body_max))
        assert all(bound(int(body_fever)) <= endpoint_bound for body_fever in body_values)
