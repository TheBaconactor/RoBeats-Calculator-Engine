...
def test_full_pipeline_signature_uses_stable_timing_envelope_context():
    ...
    # Baseline gem-solver song key is chart-structure driven and stable for identical inputs.
    assert _song_cache_key(calc_song_a) == _song_cache_key(calc_song_b)

    # Full base+FG sufficient key (deterministic timing-envelope FG streams) is stable for identical inputs.
