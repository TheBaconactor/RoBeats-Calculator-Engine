...
def test_zero_ms_tier_replay_produces_meta_and_fg_leaderboards():
    """Tier replay under zero_ms yields per-tier meta + FG leaderboards (CPU: FG 0ms == base 0ms)."""
    ...
    # Persisted surface is head-only valid for a <100-note song; under zero_ms FG is scored by the
    # base 0ms scorer (FG 0ms == base 0ms), so it only has to pass the require_response_surface guard.
    entry = {
        "score": 1,
        "fg_score": 1,
    ...
    for tier_name, tier in tiers.items():
        ...
        # FG 0ms == base 0ms: the FG loadout (force.Stats == details.Stats) scores its base 0ms value.
        assert int(tier["fg_top51"][0]["fg_score"]) == int(tier["base_top51"][0]["score"])
