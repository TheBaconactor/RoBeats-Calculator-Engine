...
from gear_optimizer.core.constants import TOTAL_ROWS, GEM_SCALE_NORMAL, GEM_SCALE_FEVER, GEM_STAT_TO_ELEMENT_SCALE, ELEMENTAL_GEM_SCALE
...
def check_parity(name, base_stats, cfg_override, song_colors):
    ...
    if not assert_eq(cpu_res["Score"], gpu_res["Score"], "Score", "CPU", "GPU"): all_passed = False
    
    # Check Gem Counts deeply
    cpu_gems = cpu_res["GemCounts"]
    gpu_gems = gpu_res["GemCounts"]
    for k in cpu_gems:
        if not assert_eq(cpu_gems[k], gpu_gems.get(k, 0), f"Gem {k}", "CPU", "GPU"): all_passed = False
        
    # Check FT/FF
    if not assert_eq(cpu_res["FT"], gpu_res["FT"], "FT Gems", "CPU", "GPU"): all_passed = False
    if not assert_eq(cpu_res["FF"], gpu_res["FF"], "FF Gems", "CPU", "GPU"): all_passed = False
    
    # Compare GPU Solver vs GPU Batch
    print("Comparing GPU Solver vs GPU Batch...")
    if batch_res:
        if not assert_eq(gpu_res["Score"], batch_res["Score"], "Score", "GPU_Solver", "GPU_Batch"): all_passed = False
        ...
        if not assert_eq(gpu_res["FT"], batch_data.get("FT"), "FT Gems", "GPU_Solver", "GPU_Batch"): all_passed = False
        if not assert_eq(gpu_res["FF"], batch_data.get("FF"), "FF Gems", "GPU_Solver", "GPU_Batch"): all_passed = False
        
        batch_gems = batch_data.get("GemCounts", {})
        for k in gpu_gems:
             if not assert_eq(gpu_gems[k], batch_gems.get(k, 0), f"Gem {k}", "GPU_Solver", "GPU_Batch"): all_passed = False
    else:
...
if __name__ == "__main__":
    base_stats = {
        ...
        "Rush": 100, "Flow": 100, "Beat": 100, "Vibe": 100, "Chill": 100,
    }
    ...
    s1 = check_parity(
        "No User Gems", 
        base_stats, 
        {"selected_color": "Rush", "user_ft": 0, "user_ff": 0, "user_pp": 0, "user_cm": 0, "user_fm": 0, "static_elem_input": 0},
        ("Rush", "Flow")
    )
    ...
    s2 = check_parity(
        ...
        base_stats, 
        {"selected_color": "Rush", "user_ft": 2, "user_ff": 2, "user_pp": 5, "user_cm": 5, "user_fm": 5, "static_elem_input": 10},
        ("Rush", "Flow")
    )
    ...
    s3 = check_parity(
        ...
        base_stats, 
        {"selected_color": "Beat", "user_ft": 0, "user_ff": 0, "user_pp": 0, "user_cm": 0, "user_fm": 0, "static_elem_input": 0},
        ("Beat", "Vibe")
    )
