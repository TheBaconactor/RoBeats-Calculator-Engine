"""FG worker contract for the fused exact Base->FG owner continuation (Slice 3).

The GPU owner scores FG in the Base turn and hands back an owner score map
...
def test_native_fg_pipeline_does_not_expose_direct_force_greats_route():
    ...
    assert not hasattr(fg_pipeline, "run_force_greats_response_frontier_for_base_candidates")
...
def test_prepare_fg_job_builds_plan_without_owner_round_trip(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda _song, *, fg_candidate_limit: ([{"candidate": 1}], 1),
    )
    monkeypatch.setattr(FgPlanner, "plan_base_candidates", staticmethod(lambda *_args, **_kwargs: plan))
    ...
    # BUILD/SCORE during prep (the owner already scored in the Base turn).
