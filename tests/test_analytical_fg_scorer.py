...
class AnalyticalFGScorer:
    ...
    def compute_baseline_timeline(self, ft_stat: int, ff_stat: int) -> Tuple[List[int], List[int], int, int, int]:
        """
    ...
    def evaluate_config(
        ...
        body_score = self.calc_body_score(base_value, combo_mul, fever_mul, count_body_fever, count_body_normal)
...
def test_full_evaluation():
    ...
    for cfg in configs:
        result = scorer.evaluate_config(cfg, ft_stat, ff_stat, pp_stat, cm_stat, fm_stat, p_val, s_val)
        print(f"  Config {cfg}:")
        print(
            f"    base_score={result['base_score']}, penalty={result['score_penalty']}, "
