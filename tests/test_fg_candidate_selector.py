...
def test_select_top_base_ga_candidates_uses_base_score_not_fg_proxy_or_diversity() -> None:
...
def test_select_top_base_ga_candidates_dedupes_loadout_by_best_base_score() -> None:
    lower = _candidate("same", 100)
    higher = _candidate("same", 200)

    selected = select_top_base_ga_candidates([lower, higher], limit=10)

    assert selected == [higher]


def test_select_top_base_ga_candidates_caps_to_top_base_limit() -> None:
    candidates = [_candidate(str(i), i) for i in range(80)]

    selected = select_top_base_ga_candidates(candidates, limit=51)
